...
class PleasantHabit(models.Model):
    ...
    last_notification = models.DateTimeField(
        verbose_name="Дата последнего напоминания", default=timezone.now
    )
    next_notification = models.DateTimeField(
        verbose_name="Дата следующего напоминания", null=True, blank=True
    )
    ...
    periodicity = models.PositiveIntegerField(
        ...
        validators=[
            MinValueValidator(1, "Привычку нельзя выполнять чаще, чем раз в день"),
            MaxValueValidator(
                7, message="Привычку можно выполнять не реже, чем раз в 7 дней"
            ),
        ],
    )
    ...
    def calculate_next_notification(self):
        ...
        next_date = datetime.combine(base_date, notify_time) - timedelta(minutes=5)
...
class UsefulHabit(models.Model):
    ...
    text_reward = models.CharField(
        max_length=250, null=True, blank=True, verbose_name="Вознаграждение"
    )
    pleasant_habit_reward = models.ForeignKey(
        to=PleasantHabit,
        on_delete=SET_NULL,
